...
class backupbuddy_migrateDB {
	...
	function bruteforceTables() {
		...
		pb_backupbuddy::status( 'details', 'Old DB prefix: `' . $old_prefix . '`; New final DB prefix (override does not apply): `' . $finalPrefix . '`. Network prefix: `' . $this->networkPrefix . '`' );
		if ($old_prefix != $finalPrefix ) {
			mysql_query("UPDATE `". $finalPrefix ."usermeta` SET meta_key = REPLACE(meta_key, '".$old_prefix."', '". $finalPrefix ."' );"); // usermeta table temporarily is in the new subsite's prefix until next step.
			pb_backupbuddy::status( 'details', 'Modified ' . mysql_affected_rows() . ' row(s) while updating meta_key\'s for DB prefix in subsite\'s [temporary if multisite] usermeta table from `' . $old_prefix . '` to `' . $finalPrefix . '`.' );
			if ( mysql_error() != '' ) { pb_backupbuddy::status( 'error', 'mysql error: ' . mysql_error() ); }

			mysql_query("UPDATE `". $finalPrefix ."options` SET option_name = '".$finalPrefix."user_roles' WHERE option_name ='".$old_prefix."user_roles' LIMIT 1");
